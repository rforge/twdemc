...
twDEMCBlockInt <- function(
	...
	ctrl = list( 
		...
		TStart=1, Tend=1, 	##<< initial and end Temperature to flatten the density surface, for each population
		widthAcceptWindow = 100, ##<< number of generations back over which the acceptance rate is calculated
	...
	#calculate temperature steps: exponentially decreasing from TStart to Tend
	if( is.null(ctrl$TStart)) ctrl$TStart=1
	if( length(ctrl$TStart) != nPops) ctrl$TStart=rep(ctrl$TStart[1],nPops)  #provide Temp for each population
	if( length(ctrl$Tend) != nPops) ctrl$Tend=rep(ctrl$Tend[1],nPops)  #provide Temp for each population
	temp[d$gen,] <- ctrl$TStart
	TstepFixed = matrix( sapply( 1:nPops, function(iPop){ pmax(1,calcDEMCTemp( ctrl$TStart[iPop], ctrl$Tend[iPop], nGen ))}), nrow=nGen, ncol=nPops)
	...
	## The acceptance rate is tracked for each chain across ctrl$widthAcceptWindow generations.
	...
	aThinWinW <- ctrl$widthAcceptWindow %/% ctrl$thin	# number of thinning intervals over which to calculate average acceptance rate
	ctrl$widthAcceptWindow <- aThinWinW * ctrl$thin
	...
	for( iThin0 in (0:(nThinnedGen-1)) ){
		...
		#calculate acceptance rate over last min(ctrl$widthAcceptWindow, iGen) generations
	...
	### \item{pAccept}{ array (nStep x nChain) the acceptance probability over previous ctrl$widthAcceptWindow steps }
