.mod2Pred <- function(
	...
	, biasFacY2=1.2	##<< model structural deficiency
){
   ...
   list( ##describe<<
		...
		,y2 = as.numeric(theta[1]*xSparce[1] + (theta[2])*xRich) * biasFacY2		 
	) ##end<<
}
...
twTwoDenEx1 <- within( list(), {
		...
		obsTrue <- .mod2Pred( thetaTrue, xSparce=xSparce,xRich=xRich, biasFacY2=1)
...
.tmp.f.plot <- function(){
	...
	with(twTwoDenEx1,{
		...
		points( obsBiased$y2 ~ xRich, col="green")
