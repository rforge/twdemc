...
mdi.kY_kO_h_dO_bias_tLag <- function(){
	# test for repeating former version, see bias scenarios below
	#using sfRemoteWrapper and exporting before 
	poptnames <- c("kY","kO","h","dO","biasLitterRoot","biasLitterLeaf","tLagLeaf","tLagRoot")
	...
	tmp.fcovarPrior <- function(){
		...
		.nPops=3
		#Zinit <- initZtwDEMCNormal( normpopt, covMat, nChains=4*.nPops, nPops=.nPops)
		...
		try(dir.create("tmp"))
		save(resMCB,file=file.path("tmp","resMCB_nonSteady2_4.RData"))
		
		plot(as.mcmc.list(resMCB))
		matplot( resMCB$rLogLik, type="l" )
		plotChainPopMoves(resMCB)
	}
	load(file.path("tmp","resMCB_nonSteady2_4.RData"))	#resMCB
	...
	windows(width=6.4,height=5.4,pointsize=10, record=TRUE)
	...
	tmp.npreg <- function(){
		...
		tmp2 <- plot2DKDMarginals( form, ds, dim=50, bandwidth=400)
		xy <- apply(do.call(cbind,dimnames(tmp2)),2,as.numeric  )
		tmp3 <- twPlot2DFun.contour( xy[,1], xy[,2], tmp2, xdiv=nrow(tmp2)
			, xlab="Rate of C-Stock change (gC/m2/yr)"
			, ylab="Turnover time of old carbon (yr)"
