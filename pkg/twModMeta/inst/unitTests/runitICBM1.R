...
test.calcSteadyHcY_ICBMDemo <- function(){
	p0 <- list(
		kY=1
		,kO=1/30
		,iY=480
	...
	p1 <- with(p0, {calcSteadyHcY_ICBMDemo(Ctot=Ctot,iY=iY,parms=p0)})
	#p0[c("h","cY")]
	#attach(p1)
	with(p1,{
	...
	#detach()
	p2 <- with(p1, {calcSteadyK_ICBMDemo(Ctot=Ctot,iY=iY,parms=p1)})
...
test.steadyStateHowland <- function(){
	...
	parms0 <- calcSteadyHcY_ICBMDemo(Ctot=Ctot,iY=sumInput,parms=parms0)
	yr0 <- 1940
	yrEnd <- 2007
	times <- yr0:yrEnd
	...
	#mtrace(initStateSoilMod)
	x0 <- initStateICBMDemo( xc12=as.vector(Ctot*c(parms0$cY,(1-parms0$cY))),iR=matrix(c(iRNew,iROld),ncol=1,dimnames=list(NULL,"c14")) )
	
	#mtrace(derivICBMDemo)
	#mtrace(solveICBMDemo)
	system.time(res <- resR <- resSolve <- solveICBMDemo(
		x0=x0,	times=times
		,parms=parms0
		,input=input
		,useRImpl=TRUE
	...
	#mtrace(solveICBMDemo)
	system.time(res <- resc <- solveICBMDemo(
		x0=x0,	times=yr0:2007	
		,parms=parms0
		,input=input
		,useRImpl=FALSE
